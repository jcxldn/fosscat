...
\begin{tikzpicture}[node distance=2cm]
    ...
    \node (actions_explain) [
        ...
        The actions module will consist of the\\
        functions that fulfil each request. It\\
        will also be responsible for validating\\
        each request and handling errors\\
        appropriately.
    };
...
\begin{tikzpicture}[node distance=2cm]
    ...
    % Validation children
    \node (error) [mb_node, right of=validation, xshift=2cm] {Error handling};
...
\begin{enumerate}
    ...
    \item \textbf{Operation} will handle all other GraphQL requests,
          ...
          (TODO: explain in 2.0.0? post link).
          \begin{enumerate}
              \item Query\\
                    \resizebox{\textwidth}{!}{
                        \begin{tikzpicture}[node distance=2cm]
