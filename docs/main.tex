...
\noindent \\ (everything past here is not super polished)

\subsubsection{Problems encountered when moving to PostgreSQL}

(JC: other non-interesting/problematic stuff happened in the meantime)
...
\subparagraph{How to switch to PostgreSQL? TBD}

\subparagraph{Entity relation errors}
...
\noindent \\ This error prevented me from progressing with the backend, as it would error out before the tables were created.(rewrite me)

\noindent \\ After some investigation, I found out that this error occurs when tables that have dependencies (what kind?) on each other are created at the same time (in the same \lstinline{AutoMigrate} call). The fix was to create the dependent table first followed by the table that depended on it, the code for which can be seen below:
...
\noindent \\ I \textbf{validated} this fix by deleting the database and having GORM recreate it on the next startup. All of the necessary tables and columns were created successfully, and no errors were produced in the console (should I show a successful startup / database table list here?)

\subparagraph{Operator does not exist}
...
\subsubsection{Placeholder: next to write up}

\begin{outline}
    \1 Error with how I defined foreign keys due to not reading the docs and setting them up incorrectly
    \2 Time-consuming to identify the problematic
    \2 Found out that SQLite does not verify foreign key constraints
    \1 Add foreign keys for lists (eg entities: Entity[])
    \2 Documented the test process for that
    \1 Empty structs/data when querying for nested elements (two tables worth of data in same query)
    \2 Fix was to load the other table.
\end{outline}

Are all of these worth talking about? Last one is probably the weakest.
